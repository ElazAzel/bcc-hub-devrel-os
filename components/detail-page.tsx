...
function CreateSubtaskModal({ open, onClose, parentId, onSaved }: { open: boolean; onClose: () => void; parentId: string; onSaved: () => void }) { const [values, setValues] = useState({ title: "", description: "", status: "", priority: "", due_date: "" }); const [saving, setSaving] = useState(false); const [error, setError] = useState(""); async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(""); try { await createSubtask(parentId, values); setValues({ title: "", description: "", status: "", priority: "", due_date: "" }); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : "Не удалось создать субзадачу."); } finally { setSaving(false); } } return <Modal open={open} onClose={onClose} title="Добавить субзадачу" description="Она будет связана с текущей задачей и войдёт в расчёт готовности."><form onSubmit={submit} className="space-y-4"><Field label="Название"><Input autoFocus value={values.title} onChange={(event) => setValues({ ...values, title: event.target.value })} placeholder="Например, собрать обратную связь" /></Field><Field label="Описание"><Textarea value={values.description} onChange={(event) => setValues({ ...values, description: event.target.value })} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Статус"><Select value={values.status} onChange={(event) => setValues({ ...values, status: event.target.value })}><option value="">Не указан</option><option>Inbox</option><option>Planned</option><option>In Progress</option><option>Waiting</option><option>Blocked</option><option>Done</option></Select></Field><Field label="Приоритет"><Select value={values.priority} onChange={(event) => setValues({ ...values, priority: event.target.value })}><option value="">Не указан</option><option>Low</option><option>Normal</option><option>High</option><option>Critical</option></Select></Field></div><Field label="Срок"><Input type="date" value={values.due_date} onChange={(event) => setValues({ ...values, due_date: event.target.value })} /></Field>{error && <p className="rounded-xl bg-[#FDECEC] px-3 py-2 text-sm text-[#AF3030]" role="alert">{error}</p>}<div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>Отмена</Button><Button type="submit" variant="brand" disabled={saving}>{saving ? "Создаём…" : "Создать субзадачу"}</Button></div></form></Modal>; }
