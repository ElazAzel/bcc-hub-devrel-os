...
import { createRecord, createRecords, findPotentialDuplicates } from "@/lib/data";
import { fieldLabel, localizeOptions, moduleCopy, ru } from "@/lib/i18n";
import { getModule, MODULES, type AnyRecord, type FieldConfig, type ModuleKey } from "@/lib/types";
...
function recordName(record: AnyRecord) { return String(record.title ?? record.name ?? ([record.first_name, record.last_name].filter(Boolean).join(" ") || "Без названия")); }
...
export function QuickAdd({ open, onClose, initialModule }: { open: boolean; onClose: () => void; initialModule?: ModuleKey }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const createdRecord = await createRecord(module, values);
      if (module === "events" && values.type === "Meetup" && meetupTemplate) {
        await createRecords("tasks", meetupChecklist.map((title) => ({ title, status: "Planned", priority: "Normal", due_date: values.date_start || undefined, source_type: "Event", source_label: recordName(createdRecord) })));
      }
